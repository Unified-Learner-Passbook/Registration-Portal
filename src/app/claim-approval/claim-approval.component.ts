  ...
  getStudentDetail(claimStatus = "pending") {
    ...
    this.generalService.postStudentData('/studentDetail', search).subscribe((res) => {
      ...
      this.studentDetails = res.result.map((item: any) => {
        return { ...item, osCreatedAt: this.generalService.getDaysDifference(item.osCreatedAt), enrollon: dayjs(item.enrolled_on, 'MM/YYYY').toDate() }
      });
      this.pageChange();
  ...
  approveStudent(user) {
    ...
    const payload = {
      ...
      "credentialSubject": {
        ...
        "school_id": this.authService.schoolDetails?.udiseCode //user?.school_udise
      }
